...
const socket = useSocket('ws://localhost:4000', {
    ...
    useEffect(() => {
      ...
      socket.on('chat_message_backend',(message) =>{
        console.log(`Message an alle: Datum${new Date(message.date)} Msg: ${message.payload}`)
        setMsglist([...msglist,{date:new Date(message.date), content:message.payload}])
        console.log(msglist)
      })


    function sendChatMsg(msg:ChatMessage){
        socket.emit("chat_message_frontend",{payload:msg.content, date:msg.date})
    }
